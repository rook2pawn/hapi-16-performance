...
internals.Request = function (connection, req, res, options) {
    ...
    this.info = {
        ...
        host: req.headers.host ? req.headers.host.replace(wsRegExp, '') : ''
    };
...
internals.Request.prototype._setTimeouts = function () {
    ...
    if (serverTimeout) {
        let now = Date.now();
        let elapsed = now - this.info.received;
        serverTimeout = Math.floor(serverTimeout - elapsed);          // Calculate the timeout from when the request was constructed
        const timeoutReply = () => {

            this._log(['request', 'server', 'timeout', 'error'], { timeout: serverTimeout, elapsed: elapsed });
